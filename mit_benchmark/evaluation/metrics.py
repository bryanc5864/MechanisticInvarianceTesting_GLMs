...
def compute_mes(
    ...
    # Handle case where both have zero variance
    if n1 + n2 <= 2:
        return 0.0
    ...
    if pooled_std == 0 or np.isnan(pooled_std):
        # If no variance, use a large effect size with sign of difference
        # Cap at 10 to avoid numerical issues in downstream analysis
...
def compute_scr(
    ...
    Returns:
        Scramble control ratio (fraction where structured > scrambled)
    """
    ll_compensated = np.asarray(ll_compensated)
    ll_scrambled = np.asarray(ll_scrambled)

    n = min(len(ll_compensated), len(ll_scrambled))
    return np.mean(ll_compensated[:n] > ll_scrambled[:n])
...
def compute_all_metrics(
    ...
    # Secondary metrics
    mes_natural = compute_mes(ll_a, ll_b)
    mes_synthetic = compute_mes(ll_c, ll_d)
    cir = compute_cir(mes_synthetic, mes_natural)
    cm = compute_cm(ll_d, ll_e, ll_c)
    scr = compute_scr(ll_e, ll_h)
